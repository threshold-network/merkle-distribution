...
// File generated by tBTC rewards script
const REWARDS_DETAILS_PATH =
  "distributions/2024-02-23/tBTCv2-rewards-details/1706745600-1708646400.json"
...
async function getTbtcRewards(legacyStakes) {
  ...
  // Let's check if the stakes comply with the rest of tBTC requirements
  const rewardsDetails = JSON.parse(fs.readFileSync(REWARDS_DETAILS_PATH))
  const rewardsRequirements = {}
  Object.keys(rewardsDetails).map((stake) => {
    rewardsRequirements[Object.keys(rewardsDetails[stake])[0]] =
      rewardsDetails[stake][Object.keys(rewardsDetails[stake])[0]].requirements
  })

  const filtRewards = {}
  Object.keys(rewards).map((stake) => {
    const stakeAddress = ethers.utils.getAddress(stake)
    const requirements = rewardsRequirements[stakeAddress]
    if (
      requirements &&
      requirements.isPreParamsSatisfied &&
      requirements.isUptimeSatisfied &&
      requirements.isVersionSatisfied
    ) {
      filtRewards[stake] = rewards[stake]
    }
  })
